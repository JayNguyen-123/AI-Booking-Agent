...
from tools import flights_finder
...
TOOLS_SYSTEM_PROMPT = f""" You are a smart travel agency. Use the tools to look up information.
        ...
        In your output always include the price of the flights and the price of the hotel and the currency as well (if possible).
...
EMAIL_SYSTEM_PROMPT = """ Your task is to convert structured markdown-like text into a valid email body.

_ Do not include a '''html preamble in your response.
- The output should be in proper HTML format, ready to be used as the body of an email.
Here is an example:
<example>
Input:

I want to travel to New York from Madrid from October 1-6. Find me flights and 4-star hotels.
...
class Agent:
    def __init__(self):
        ...
        self._tools_llm = ChatAnthropic(model="claude-3-5-sonnet-20240620").bind_tools(TOOLS)
    ...
    def email_sender(self, state: AgentState):
        print("Sending email")
        email_llm = ChatAnthropic(model="claude-3-5-sonnet-20240620", temperature=0.1)
        email_message = [SystemMessage(content=EMAIL_SYSTEM_PROMPT), HumanMessage(content=state["messages"][-1].content)]
        email_response = email_llm.invoke(email_message)
        print("Email content: ", email_response.content)

        message = Mail(from_email=os.environ["FROM_EMAIL"], to_email=os.environ["TO_EMAIL"], subject=os.environ["EMAIL_SUBJECT"],
